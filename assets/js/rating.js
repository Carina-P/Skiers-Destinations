...
function fillDocuTable(){
    resortsRating.forEach((ratingRow, i) => {
        let id = "#index"+i;
...
function updateTable(event) { 
    ...
    resortsRating[event.data.index]= calcNewRating( resortsRating[event.data.index], parseInt(this.value));
    
    resortsRating.sort( (a,b) => {return b.rating-a.rating;})
...
function fetchTableData(){
    resortsRating = JSON.parse(localStorage.getItem("ratingTable")) || initTable(); 
    resortsRating.forEach( row => {row.myVote = 0;}); 
    fillDocuTable(); 
}
